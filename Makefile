...
deploy: push
	...
	sed -i 's/{IMAGE_TAG_for_change}/$(TAG)/g' deploy/matplot-service-deployment.yaml
	# apply change
	kubectl apply -f deploy/ --namespace "$(DEPLOY_NAMESPACE)"
	# restore deployment.yaml
	sed -i 's/$(TAG)/{IMAGE_TAG_for_change}/g' deploy/matplot-service-deployment.yaml
